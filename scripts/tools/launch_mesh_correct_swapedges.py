...
def create_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Read a .msh mesh file, swaps edges of specified cells, "
            "and save the corrected mesh in Tolosa format."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python correct_mesh.py --input_mesh my_file.msh --cells 2836 2838 2836 2838 2850 2851 5033 5226\n"
            "  python correct_mesh.py --input_mesh my_file.msh --cells 2836 2838 -n gmsh -v\n"
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Flat list of cell pairs to swap edges, given as a sequence of integers. "
            "Must contain an even number of values, each consecutive pair (a, b) "
            "defines one edge swapping operation. "
            "Example: --cells 2836 2838 2850 2851"
        ),
    )
...
def parse_merge_cells(flat_list):
    """Convert a flat list of integers into an (N, 2) numpy array of cell pairs."""
    if len(flat_list) % 2 != 0:
        raise ValueError(
            f"--cells requires an even number of values; got {len(flat_list)}."
        )
...
def build_output_filename(input_mesh):
    """Append '_swapedges' before the file extension of the input mesh filename."""
    base, ext = os.path.splitext(os.path.basename(input_mesh))
    return f"{base}_swapedges{ext}"
...
def main():
    ...
    try:
        cells = parse_merge_cells(args.cells)
    ...
    p = processor.swap_edges_cells(cells, numbering=args.numbering)
